...
int main(void) {
    ...
    char *sentence = "java logic arrays";
    int count = 0;
    for (char *word : sentence.split("\\s+")) if (word.indexOf('a') >= 0) count++;
    printf("%s", "Count = ");
 ...
 * The condition `word.indexOf('a') >= 0` decides whether the current value matches the requirement.
